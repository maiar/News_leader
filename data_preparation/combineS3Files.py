...
def run_concatenation(folder_to_concatenate, result_filepath, file_suffix, max_filesize):
    ...
    for i, parts in enumerate(grouped_parts_list):
        ...
        ind = result_filepath.find('.')
        result_filepath = result_filepath[:ind] + '-' + str(i) + result_filepath[ind:]
        print(result_filepath)
        run_single_concatenation(s3, parts, result_filepath)
...
def collect_parts(s3, folder, suffix):
    filesList = _list_all_objects_with_size(s3, folder)

    return list(filter(lambda x: x[0].endswith(suffix), filesList))
